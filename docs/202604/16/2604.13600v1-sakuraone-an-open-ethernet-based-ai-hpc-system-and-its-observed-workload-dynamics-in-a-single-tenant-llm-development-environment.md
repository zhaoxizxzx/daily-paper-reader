---
title: "SAKURAONE: An Open Ethernet-Based AI HPC System and Its Observed Workload Dynamics in a Single-Tenant LLM Development Environment"
title_zh: SAKURAONE：一种基于开放以太网的 AI 高性能计算系统及其在单租户 LLM 开发环境中的工作负载动态观测
authors: "Fumikazu Konishi, Yuuki Tsubouchi, Hirofumi Tsuruta"
date: 2026-04-15
pdf: "https://arxiv.org/pdf/2604.13600v1"
tags: ["query:netmes"]
score: 6.0
evidence: 开放以太网AI HPC系统中的工作负载动态观测
tldr: SAKURAONE是SAKURA互联网研究中心开发的AI HPC集群，在TOP500中排名第49位。它是全球前100强中唯一采用全开放网络栈（800 GbE与SONiC）的系统，包含100个配备8路H100的节点。研究通过分析单一租户的大模型开发负载，揭示了从大规模初始训练向中规模迭代优化的资源利用动态，证明了开放以太网技术在超大规模AI计算中的可扩展性。
selection_source: fresh_fetch
motivation: 旨在构建一个基于开放网络技术的高性能AI计算集群，以支持大规模语言模型训练并探索其在真实工作负载下的运行动态。
method: 采用基于SONiC的800 GbE开放以太网架构和RoCEv2协议构建100节点H100集群，并对单一租户的LLM开发全周期任务进行观测分析。
result: 系统实现了33.95 PFLOP/s的HPL性能，并观察到资源消耗从初期的少数大规模任务逐渐转向后期的中等规模迭代任务。
conclusion: SAKURAONE证明了供应商中立的开放以太网技术在顶级HPC系统中的可行性，并揭示了LLM开发过程中GPU资源利用的演进规律。
---

## 摘要
SAKURAONE 是由 SAKURA Internet 研究所开发并运营的托管型高性能计算（HPC）集群。它基于 KOKARYOKU PHY 裸金属 GPU 平台构建，并针对包括大语言模型（LLM）训练在内的高级工作负载进行了优化。在 ISC 2025 TOP500 榜单中，SAKURAONE 的 HPL 排名位列第 49 位，是前 100 强中唯一采用全开放网络栈（基于 SONiC 的 800 GbE）的系统，展示了厂商中立技术的可扩展性。实测性能达到 33.95 PFLOP/s (HPL Rmax)、396.295 TFLOP/s (HPCG)，以及在 FP8 精度下的 HPL-MxP 性能为 339.86 PFLOP/s。该系统包含 100 个节点，每个节点配备 8 块 NVIDIA H100 GPU 和 2 PB 全闪存 Lustre 文件系统，通过采用 RoCEv2 的轨道优化（rail-optimized）800 GbE 叶脊拓扑网络互连。通过单个研究项目的独占使用，我们观测到了开发相关作业的特征。与以往的 HPC 研究一致，小规模作业在数量上占主导地位，而少数大规模作业则占据了大部分 GPU 资源时间。随着项目的推进，资源使用从大规模作业转向中等规模作业，反映了从初始大规模训练到迭代优化的转变。这些观测结果阐明了在统一项目工作负载下，GPU 集群在真实世界中的利用率动态。

## Abstract
SAKURAONE is a managed high performance computing (HPC) cluster developed and operated by the SAKURA Internet Research Center. It builds on the KOKARYOKU PHY bare metal GPU platform and is optimized for advanced workloads, including large language model (LLM) training. In ISC 2025 TOP500, SAKURAONE is ranked 49th by HPL and is the only top 100 system that uses a fully open networking stack - 800 GbE with SONiC - demonstrating the scalability of vendor-neutral technology. Measured performance is 33.95 PFLOP/s (HPL Rmax), 396.295 TFLOP/s (HPCG), and 339.86 PFLOP/s on HPL-MxP with FP8. The system consists of 100 nodes, each with eight NVIDIA H100 GPUs and a 2 PB all-flash Lustre file system, interconnected via a rail-optimized 800 GbE leaf-spine fabric with RoCEv2. Through exclusive use by a single research project, we observed the characteristics of development-related jobs. Consistent with previous HPC studies, small-scale jobs dominated in number, while a few large-scale jobs accounted for most GPU resource time. As the project progressed, resource use shifted from large-scale to mid-scale jobs, reflecting a transition from initial large-scale training to iterative refinement. These observations illustrate the real-world utilization dynamics of GPU clusters under unified project workloads.