...
title_zh: 验证网内计算系统的设计风险
authors: "Tianyu Bai, Ying Zhang, Xiaoxi Zhang, Wenfei Wu"
date: 2026-04-11
pdf: "https://arxiv.org/pdf/2604.10186v2"
tags: ["query:netmes"]
score: 7.0
evidence: 验证可编程交换机上的网络内计算
tldr: 针对可编程交换机带来的网络内计算（INC）系统在处理丢包、乱序等网络异常时存在的正确性风险，本文提出了首个通用验证工具INCGuard。该工具通过高层规范语言简化建模，利用模型检测技术识别设计缺陷，并针对INC场景进行状态空间优化。实验表明，INCGuard能在数秒内发现7个现有INC系统的设计风险，有效保障了系统的可靠性。
selection_source: fresh_fetch
motivation: 网络内计算打破了传统网络的透明性，网络异常可能导致应用属性违背，而传统测试难以穷尽这些异常。
method: 提出INCGuard工具，通过高层规范语言建模，将系统转化为状态转移表示，并利用优化的模型检测技术探测设计风险。
result: 成功对7个INC系统进行了建模，在数秒内识别出潜在风险，并在真实系统中复现验证了结果的有效性。
conclusion: INCGuard为INC系统的设计验证提供了高效、通用的解决方案，显著降低了开发者的建模负担并提升了系统安全性。
---

## 摘要
近年来，可编程交换机的出现使网内计算（INC）成为关注焦点。通过将计算直接卸载到数据传输过程中，INC 提高了网络利用率，将延迟降低至亚 RTT 级别，节省了链路带宽，并维持了吞吐量。然而，INC 破坏了传统网络的透明性，迫使开发者必须考虑丢包和乱序等网络异常情况。如果处理不当，这些异常可能导致违反应用属性，例如缓存一致性和锁互斥。常规测试无法穷尽这些异常，引发了对 INC 系统正确性的质疑，并阻碍了其在工业界的部署。本文提出了 INCGuard，这是首个用于验证 INC 系统的通用工具。INCGuard 提供了一种高级规范语言，平均为开发者节省了 67.2% 的代码行数。为了帮助更好地理解系统行为，INCGuard 提供了可配置的网络环境，并允许开发者表达 INC 特有的正确性属性。INCGuard 将开发者指定的系统转换为状态转移表示，执行模型检测以发现潜在的设计风险，并向开发者报告违规轨迹。我们针对 INC 特有场景提出了优化方案，以应对状态空间爆炸的挑战。我们对七个 INC 系统进行了建模，并使用 INCGuard 在数秒内识别出了它们的风险。我们进一步在真实系统中复现了这些风险，以确认验证结果的有效性。
