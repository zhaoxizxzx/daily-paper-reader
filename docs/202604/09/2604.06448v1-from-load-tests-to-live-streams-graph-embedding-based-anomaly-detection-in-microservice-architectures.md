...
tags: ["query:netmes"]
score: 7.0
evidence: 微服务流量图中的异常检测
tldr: Prime Video针对直播和点播流量高峰，提出一种基于图嵌入的微服务异常检测系统。该系统利用GCN-GAE从有向加权服务图中学习节点级表示，通过对比压力测试与实际活动期间的嵌入向量余弦相似度来识别异常服务。研究证明了其在识别故障相关服务和早期预警方面的有效性，并提供了合成异常评估框架。
selection_source: fresh_fetch
motivation: 传统的压力测试有时无法完全模拟真实活动中独特的微服务行为，导致难以发现潜在的系统异常。
method: 采用基于图卷积网络（GCN）的图自动编码器（GAE），在分钟级分辨率的有向加权服务图上学习无监督节点嵌入。
result: "系统成功识别了已记录的故障相关服务，并在合成评估中达到96%的精确率和0.08%的极低误报率。"
conclusion: 该框架在Prime Video实际场景中具有实用价值，为微服务生态系统中的异常检测提供了基于图表示学习的基础。
---

## 摘要
Prime Video 定期进行负载测试，以模拟在《周四橄榄球之夜》（Thursday Night Football）等直播活动以及《力量之戒》（Rings of Power）等视频点播（VOD）活动中出现的观众流量激增。虽然这些压力测试验证了系统容量，但有时会遗漏真实活动流量中特有的服务行为。我们提出了一种基于图的异常检测系统，该系统利用无监督节点级图嵌入来识别表现不足的服务。我们的方法基于 GCN-GAE 构建，从分钟级分辨率的有向加权服务图中学习结构表示，并根据负载测试嵌入与活动嵌入之间的余弦相似度来标记异常。该系统识别出了已记录的与事件相关的服务，并展示了早期检测能力。我们还引入了一个初步的合成异常注入框架用于受控评估，结果显示出极高的准确率（96%）和较低的误报率（0.08%），尽管在保守的传播假设下召回率（58%）仍然有限。该框架展示了在 Prime Video 内部的实际效用，同时也揭示了方法论上的经验和方向，为在微服务生态系统中更广泛的应用奠定了基础。
